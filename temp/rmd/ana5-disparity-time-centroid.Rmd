...
```{r centroid-disparity-analyses, cache = TRUE}
## Set up function to calculate disparity in parallel
parDispRityCen <- function(x) {
    dispRity(x, metric = c(centroids))
}

## For parallelisation: create cluster
clusDisparity <- makeCluster(no_cores)
## Export required variables
clusterExport(clusDisparity, c("dispRity", "centroids"))

## Calculate disparity in epochs
disparityCen_epoch <- parLapply(clusDisparity, bootstrapped_epoch, parDispRityCen)
## Calculate disparity fro bJ binning
disparityCen_bJ <- parLapply(clusDisparity, bootstrapped_bJ, parDispRityCen)
...
```{r centroid-summarise-disparity}
...
data_disparityCen_epoch <- lapply(disparityCen_epoch, myDispRity.Summary)
## Apply summary to the bJ disparity lists
data_disparityCen_bJ <- lapply(disparityCen_bJ, myDispRity.Summary)
```

The values of disparity can be seen in the following table.

```{r centroid-disparity-tables-epoch, collapse = TRUE, results = "asis"}
## Make table to display disparity values for epoch bins
cat(paste("With negative eigenvalue correction", collapse = "\n"))
pander(data_disparityCen_epoch[1:4])
cat(paste("Without negative eigenvalue correction", collapse = "\n"))
pander(data_disparityCen_epoch[5:8])
```

Then the same for the 10 Ma bJ binning.

```{r centroid-disparity-tables-bJ, collapse = TRUE, results = "asis"}
cat(paste("With negative eigenvalue correction", collapse = "\n"))
pander(data_disparityCen_bJ[5:8])
cat(paste("Without negative eigenvalue correction", collapse = "\n"))
pander(data_disparityCen_bJ[5:8])
```

Then calculate the disparity on the rarefied subsamples.

```{r centroid-rarefied-disparity-analyses, eval = TRUE, cache = TRUE}
## For parallelisation: create cluster
clusDisparity <- makeCluster(no_cores)
## Export required variables
clusterExport(clusDisparity, c("dispRity", "centroids"))
...
## Calculate disparity in epochs
rare_disparityCen_epoch <- parLapply(clusDisparity, rare_bootstrapped_epoch, parDispRityCen)
## Calculate disparity fro bJ binning
rare_disparityCen_bJ <- parLapply(clusDisparity, rare_bootstrapped_bJ, parDispRityCen)

## Release cluster cores
stopCluster(clusDisparity)

## Summarise the data
data_rare_dispCen_epoch <- lapply(rare_disparityCen_epoch, myDispRity.Summary)
data_rare_dispCen_bJ <- lapply(rare_disparityCen_bJ, myDispRity.Summary)
```


Now I plot the rarefaction curves using the inbuilt `plot` function of `dispRity`, which is easier than doing it by hand.

```{r centroid-rarefy-plot, collapse = TRUE, eval = TRUE}
## Plot rarefied data for the epoch bins
# pdf("fig/rarefied-Cen-data.pdf", width = 7, height = 7)
...
plotAxisLabels <- function() {
	...
	mtext("Centroid distance", side = 2, line = 2, outer = TRUE, adj = c(0.5, 0.5), cex = 0.8, font = 2)
}


for (dist in 1:length(rare_disparityCen_epoch)) {
    print(dist)
    plot(rare_disparityCen_epoch[[dist]], rarefaction = TRUE, cent.tend = mean, quantile = 95, xlab = "", ylab = "")
    plotAxisLabels()
    mtext(paste0(plotNames[dist], ": epoch-level bins"), side = 3, line = 2, outer = TRUE, adj = c(0.5, 0.5), cex = 1.0, font = 2)
}
for (dist in 1:length(rare_disparityCen_bJ)) {
    print(dist)
    plot(rare_disparityCen_bJ[[dist]], rarefaction = TRUE, cent.tend = mean, quantile = 95, xlab = "", ylab = "")
    plotAxisLabels()
...
for(nPlot in 1:length(data_disparityCen_epoch[1:4])) {
	nUcPlot <- nPlot + 4
    ## Plot corrected data in red
    plot(bin_means_epoch[1:8], 
        data_disparityCen_epoch[[nPlot]][, "bs.mean"], 
        xlim = rev(range(disparity_bins$epoch)), 
        ylim = range(0, data_disparityCen_epoch[[nPlot]][, "2.5%"], data_disparityCen_epoch[[nPlot]][, "97.5%"]), 
        type = "n", xaxt = "n")
    bin.shading.epoch()
    arrows(bin_means_epoch + 1.5, 
        data_disparityCen_epoch[[nPlot]][, "2.5%"], 
        bin_means_epoch + 1.5, 
        data_disparityCen_epoch[[nPlot]][, "97.5%"], 
        length = 0.02, 
        angle = 90, 
        code = 3, 
        col = "red")
    ...
    par(new = TRUE)
    plot(bin_means_epoch[1:8], 
        data_disparityCen_epoch[[nUcPlot]][, "bs.mean"], 
        xlim = rev(c(66, 252.17)), 
        ylim = range(0, data_disparityCen_epoch[[nUcPlot]][, "2.5%"], data_disparityCen_epoch[[nUcPlot]][, "97.5%"]), 
        ann = FALSE, axes = FALSE, type = "n")
    arrows(bin_means_epoch - 1.5, 
        data_disparityCen_epoch[[nUcPlot]][, "2.5%"], 
        bin_means_epoch - 1.5, 
        data_disparityCen_epoch[[nUcPlot]][, "97.5%"], 
        length = 0.02, 
        angle = 90, 
        code = 3, 
        col = "blue")
...
mtext("Disparity (centroid distance; negative eigenvalues corrected)", side = 2, line = 2, outer = TRUE, adj = c(0.5, 0.5), cex = 0.8)
mtext("Disparity (centroid distance; negative eigenvalues not corrected)", side = 4, line = 2, outer = TRUE, adj = c(0.5, 0.5), cex = 0.8)
...
## Set up plot area
# dev.new(width = 5, height = 12)
# cairo_pdf("fig/disparity-time-bJ-centroids.pdf", width = 5, height = 12)
par(mfrow = c(4, 1), pty = "m", mar = c(0, 0, 0, 0), oma = c(8, 4, 1, 4), mgp = c(1.5, 0.5, 0), lheight = 0.5)

for(nPlot in 1:length(data_disparityCen_bJ[1:4])) {
	nUCPlot <- nPlot + 4
    ## Plot corrected data in red
    plot(bin_means_bJ, 
        data_disparityCen_bJ[[nPlot]][, "bs.mean"], 
        xlim = rev(range(disparity_bins$tenMa_bJ)), 
        ylim = range(0, data_disparityCen_bJ[[nPlot]][, "2.5%"], data_disparityCen_bJ[[nPlot]][, "97.5%"]), 
        type = "n", 
        xaxt = "n")
    bin.shading.bJ()
    arrows(bin_means_bJ + 1.5, 
        data_disparityCen_bJ[[nPlot]][, "2.5%"], 
        bin_means_bJ + 1.5,
        data_disparityCen_bJ[[nPlot]][, "97.5%"], 
        length = 0.02, 
        angle = 90, 
        code = 3, 
        col = "red")
    ...
    par(new = TRUE)
    plot(bin_means_bJ, 
        data_disparityCen_bJ[[nUCPlot]][, "bs.mean"], 
        xlim = rev(range(disparity_bins$tenMa_bJ)), 
        ylim = range(0, data_disparityCen_bJ[[nUCPlot]][, "2.5%"], 
            data_disparityCen_bJ[[nUCPlot]][, "97.5%"]), 
        ann = FALSE, axes = FALSE, type = "n")
    arrows(bin_means_bJ - 1.5, 
        data_disparityCen_bJ[[nUCPlot]][, "2.5%"], 
        bin_means_bJ - 1.5, 
        data_disparityCen_bJ[[nUCPlot]][, "97.5%"], 
        length = 0.02, 
        angle = 90, 
